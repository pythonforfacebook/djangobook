...
class FacebookMiddleware():
    ...
    def process_request(self, request):
        ...
        if ENABLED_PATHS and DISABLED_PATHS:
            raise ImproperlyConfigured('You may configure either DJANGOBOOK_ENABLED_PATHS or DJANGOBOOK_DISABLED_PATHS, but not both.')
...
class FacebookAuthMiddleware():
    """ Attempt to authenticate the user to any facebook enabled backends.

    The user however is not "logged in" via login() as we need to limit the
    session to facebook_data['expires']. As we can't do that, we'll just
    re-authenticate on every request.
    """
    def process_request(self, request):
        if request.user.is_anonymous() and getattr(request, 'facebook', False):
            if getattr(request.facebook, 'user', False):
               if request.facebook.user.authorized:
                  user = authenticate(facebook_id=request.facebook.user.facebook_id)
                  if user:
